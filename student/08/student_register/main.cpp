...
bool is_student_number_existed(const string&stu_num , const map<string,Student*>& mp ){
    if(mp.find(stu_num)==mp.end()){
...
void change_num(map<string,Student*>&student_numbers,const string& stu_num,const string& new_num){
   (* student_numbers.at(stu_num)).phone_number=new_num;
}
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            if(parts.size() != 2){
                ...
                if(is_student_number_existed(parts[1],student_numbers)){
                    string new_num;
