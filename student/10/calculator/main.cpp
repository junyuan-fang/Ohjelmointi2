...
#include <iostream>
#include <iomanip>
#include <sstream>  // string stream. for implementing function string_to_double
#include <string>
#include <vector>
#include <cctype>
...
int main() {
    ...
    while ( true ) {
        ...
        string ::iterator char_item= command_to_be_executed.begin();
        while (char_item!=command_to_be_executed.end()){
            *char_item=toupper(*char_item);
            ++char_item;
        }
        ...
        if(command_ptr->exit==true){
            cout<<GREETING_AT_END<<endl;
            exit(0);
        }
        else{
            cout<<command_ptr->action(parameters[0],parameters[1])<<endl;
